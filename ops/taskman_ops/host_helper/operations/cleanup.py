...
def cleanup(request: HostRequest) -> HostResult:
    ...
    try:
        ...
        with lifecycle_lock(paths, timeout_seconds=_LOCK_TIMEOUT_SECONDS):
            ...
            if action == "inspect":
                if requested:
                    return HostResult.for_request(request, "refused", "inspection must not include cleanup targets", _state_projection(state))
                return HostResult.for_request(
                    request,
                    "succeeded",
...
def _delete_target(target: Mapping[str, object], state: HostState, paths: ManagedPaths) -> bool:
    ...
    if kind == "backup":
        ...
        record = next((item for item in state.backups if item.backup_id == identifier), None)
        if path != expected or record is None:
