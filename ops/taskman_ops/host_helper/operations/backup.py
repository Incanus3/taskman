...
def backup(request: HostRequest) -> HostResult:
    ...
    try:
        ...
        return HostResult.for_request(request, "manual", "backup authority is contradictory", _state_projection(state))
    except (PathAuthorityError, BackupCapacityError, TypeError, ValueError):
        return HostResult.for_request(request, "refused", "backup request is unsafe", _state_projection(state))
