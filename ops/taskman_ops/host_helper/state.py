...
class HostState:
    ...
    successful_release_ids: frozenset[str] = frozenset()

    def __post_init__(self) -> None:
        if self.selected_release_id is not None and not isinstance(
            self.selected_release_id, str
        ):
        ...
        if not isinstance(self.releases, tuple) or not all(
            isinstance(item, ReleaseRecord) for item in self.releases
        ):
            raise StateAmbiguityError("invalid release state")
        if not isinstance(self.backups, tuple) or not all(
            isinstance(item, BackupRecord) for item in self.backups
        ):
        ...
        if self.restore_target is not None and not isinstance(
            self.restore_target, RestoreTarget
        ):
        ...
        if not isinstance(self.retiring_backup_protections, tuple) or not all(
            isinstance(item, BackupProtection)
            for item in self.retiring_backup_protections
        ):
        ...
        if (
            ...
            or not all(
                _SELECTION_FILE_RE.fullmatch(item) for item in self.selection_filenames
            )
        ):
        ...
        if not isinstance(self.successful_backup_ids, frozenset) or not all(
            type(item) is str and BACKUP_ID_RE.fullmatch(item)
            for item in self.successful_backup_ids
        ):
            raise StateAmbiguityError("invalid successful backup references")
        if not self.successful_release_ids and self.selections:
            object.__setattr__(
                self,
                ...
                frozenset(
                    ...
                    for release_id in (
                        ...
                        *(
                            (selection.previous_release_id,)
                            if selection.previous_release_id
                            else ()
                        ),
                        *(
                            (selection.observed_previous_release_id,)
                            if selection.observed_previous_release_id
                            else ()
                        ),
                    )
                ),
        ...
        try:
            valid_release_references = isinstance(
                self.successful_release_ids, frozenset
            ) and all(validate_release_id(item) for item in self.successful_release_ids)
        except (TypeError, ValueError):
            valid_release_references = False
        if not valid_release_references:
            raise StateAmbiguityError("invalid successful release references")
    ...
    def previous_successful_selection_filename(self) -> str | None:
        return (
            self.selection_filenames[-2] if len(self.selection_filenames) > 1 else None
        )
    ...
    def to_mapping(self) -> dict[str, object]:
        return {
            ...
            "backup_protections": [
                item.to_mapping() for item in self.backup_protections
            ],
            "restore_target": None
            if self.restore_target is None
            else self.restore_target.to_mapping(),
        }
...
class _SuccessfulHistory:
    ...
    def record(self, filename: str, deadline: float) -> SelectionRecord | None:
        ...
        return (
            None
            if row is None
            else SelectionRecord.from_mapping(json.loads(str(row[0])))
        )
...
def observe_host_state(
    ...
    *,
    ...
    full_temporary_paths: bool = False,
    ...
    if type(full_temporary_paths) is not bool:
        raise TypeError("full temporary-path projection flag must be boolean")
    if deadline is None:
    ...
    if restore_target is not None:
        ...
        if restore_target.replacement is not None:
            optional_restore_inputs.add(str(restore_target.replacement["backup_id"]))
        optional_restore_inputs.difference_update(
            str(item["backup_id"]) for item in restore_target.safety_backup_attempts
        )
    ...
    try:
        ...
        temporary[:] = [
            item for item in temporary if Path(item) not in retiring_dump_paths
        ]
        _require_history_deadline(float(deadline))
        _validate_backup_sources(backup_by_id, release_by_id, float(deadline))
        _validate_selection_history(
            history, release_by_id, backup_by_id, float(deadline)
        )
        ...
        selected_from_history = (
            history.projection[-1].release_id if history.projection else None
        )
    except Exception:
        history.close()
        raise
    try:
        ...
        if selected_from_history != selected_from_link:
            if selected_from_history is None and selected_from_link is None:
                ...
                raise StateAmbiguityError(
                    "current selection contradicts completed selection history"
                )
        else:
            selected = selected_from_history

        applied_migrations, database_state, initial_database_empty = _database_state(
            database
        )
        ...
        return HostState(
            ...
            temporary_paths=tuple(
                temporary if full_temporary_paths else temporary[:MAX_TEMPORARY_PATHS]
            ),
            warnings=tuple(warnings),
            backup_protections=tuple(
                sorted(backup_protections, key=lambda item: item.backup_id)
            ),
            restore_target=restore_target,
            selection_filenames=history.projection_filenames,
            successful_backup_ids=history.successful_backup_ids,
            successful_release_ids=history.successful_release_ids,
...
def _read_releases(
    ...
    for entry in entries:
        ...
        try:
            ...
            try:
                ...
                if stat.S_ISDIR(entry_details.st_mode):
                    ...
                    if stat.S_ISLNK(manifest_details.st_mode) or stat.S_ISREG(
                        manifest_details.st_mode
                    ):
                        raise StateAmbiguityError("unsupported release authority")
            except FileNotFoundError:
                pass
                ...
                raise StateAmbiguityError(
                    "unable to inspect release authority"
                ) from error
        ...
        if result[-1].release_id != release_id:
            raise StateAmbiguityError(
                "release manifest identity conflicts with its path"
            )
    return result
...
def _read_backups(
    ...
    *,
    ...
    for entry in entries:
        ...
        if entry.name.endswith(".dump"):
            ...
            if BACKUP_ID_RE.fullmatch(identifier) is not None:
                try:
                    ...
                    raise StateAmbiguityError(
                        "unable to inspect backup manifest"
                    ) from error
        ...
        try:
            record = _read_record(
                entry, BackupRecord.from_mapping, "backup manifest", owner_uid
            )
        except StateAmbiguityError:
            raise
        ...
        if record.backup_id != entry.stem:
            raise StateAmbiguityError(
                "backup manifest identity conflicts with its path"
            )
        ...
        try:
            ...
            warnings.append(f"completed backup dump is unavailable: {record.backup_id}")
        ...
        if record.backup_id in optional_dump_ids:
            try:
                available = (
                    details.st_size > 0 and sha256_file(dump) == record.dump_sha256
                )
            except OSError:
                available = False
            if not available:
    ...
    try:
        ...
        for entry in _selection_entries(root, owner_uid, deadline):
            ...
            if not _SELECTION_FILE_RE.fullmatch(entry.name):
                ...
                continue
            record = _read_record(
                entry, SelectionRecord.from_mapping, "selection record", owner_uid
            )
            _require_history_deadline(deadline)
            _validate_selection_record_references(record, releases, backups)
            if entry.name != selection_filename(record):
                raise StateAmbiguityError(
                    "selection record identity conflicts with its path"
                )
            identity = json.dumps(
                [
                    record.release_id,
            ...
            payload = json.dumps(
                record.to_mapping(), separators=(",", ":"), sort_keys=True
            )
            try:
                database.execute(
...
def _read_backup_protections(
    ...
    for entry in entries:
        ...
        if record.backup_id != entry.stem:
            raise StateAmbiguityError(
                "backup protection identity conflicts with its path"
            )
        seen.add(record.backup_id)
        result.append(record)
    return result
...
def _entries(root: Path, owner_uid: int, label: str) -> tuple[Path, ...]:
    ...
    try:
        ...
        with os.scandir(root) as entries:
            for index, entry in enumerate(entries, start=1):
                if index > MAX_INVENTORY_ENTRIES:
                    raise StateAmbiguityError(f"{label} inventory exceeds safe bound")
...
def _validate_selection_history(
    ...
    successful_release_ids: set[str] = set()
    for index, (filename, selection) in enumerate(history.ordered_rows(deadline)):
        ...
        if (
            ...
            raise StateAmbiguityError(
                "selection references an unknown observed release"
            )
        if any(backup_id not in backups for backup_id in selection.recovery_backup_ids):
            raise StateAmbiguityError("selection references an unknown recovery backup")
        if index == 0 and selection.previous_release_id is not None:
            raise StateAmbiguityError(
                "first selection cannot name a previous successful release"
            )
        ...
        if index and selection.observed_previous_release_id is None:
            raise StateAmbiguityError(
                "later selection must record its observed previous release"
            )
        ...
        successful_release_ids.add(selection.release_id)
        if selection.previous_release_id is not None:
            successful_release_ids.add(selection.previous_release_id)
        if selection.observed_previous_release_id is not None:
            successful_release_ids.add(selection.observed_previous_release_id)
    ...
    history.successful_release_ids = frozenset(successful_release_ids)
    _require_history_deadline(deadline)
...
def _validate_backup_sources(
    ...
    for backup in backups.values():
        ...
        if (
            backup.migration_versions
            != source_versions[: len(backup.migration_versions)]
        ):
            raise StateAmbiguityError(
                "backup migration provenance contradicts its source release"
            )
...
def _validate_release_fingerprint_agreement(
    ...
    for version in versions:
        if source_fingerprints.get(version) != target_fingerprints.get(version):
            raise StateAmbiguityError(
                "backup protection migration fingerprints conflict"
            )
...
def _validate_backup_protections(
    ...
    for protection in protections:
        ...
        if target is None:
            raise StateAmbiguityError(
                "backup protection references an unknown target release"
            )
        if protection.base_selection_id is None:
            if (
                first_selection
                and protection.backup_id not in first_selection.recovery_backup_ids
            ):
                raise StateAmbiguityError(
            ...
            if baseline is None:
                raise StateAmbiguityError(
                    "backup protection references an unknown selection"
                )
            successor = history.successor(protection.base_selection_id, deadline)
            if (
                successor is not None
                and protection.backup_id not in successor.recovery_backup_ids
            ):
                raise StateAmbiguityError(
        ...
        if (
            backup.migration_versions
            != target_versions[: len(backup.migration_versions)]
        ):
            raise StateAmbiguityError(
                "backup protection migration provenance is contradictory"
            )
        source = releases.get(backup.source_release_id)
        if source is None:
            raise StateAmbiguityError(
                "backup protection source provenance is unavailable"
            )
        _validate_release_fingerprint_agreement(
            source, target, backup.migration_versions
        )

        attempts = attempts_by_baseline.setdefault(protection.base_selection_id, set())
        if protection.attempt_number in attempts:
            raise StateAmbiguityError(
                "backup protection attempt numbers are duplicated"
            )
    ...
    for attempt_numbers in attempts_by_baseline.values():
        ...
        if 0 not in attempt_numbers:
            raise StateAmbiguityError(
                "backup protection is missing its original attempt"
            )
...
def _validate_retiring_backup_protections(
    ...
    for protection in retiring:
        ...
        if target is None:
            raise StateAmbiguityError(
                "retiring protection references an unknown target release"
            )
        if (
            ...
            raise StateAmbiguityError(
                "retiring protection references an unknown selection"
            )
        backup = backups.get(protection.backup_id)
        if backup is not None:
            ...
            if (
                ...
                raise StateAmbiguityError(
                    "retiring protection migration provenance is contradictory"
                )
            source = releases.get(backup.source_release_id)
            if source is None:
                raise StateAmbiguityError(
                    "retiring protection source provenance is unavailable"
                )
            _validate_release_fingerprint_agreement(
                source, target, backup.migration_versions
            )
...
def _validate_restore_target(
    ...
    if (
        ...
        raise StateAmbiguityError(
            "restore target references an unknown observed release"
        )
    ...
    for attempt in target.safety_backup_attempts:
        ...
        if backup is None:
            raise StateAmbiguityError(
                "restore target references an unknown safety attempt"
            )
    if target.replacement is not None:
        ...
        if replacement_backup is None:
            raise StateAmbiguityError(
                "restore target references an unknown replacement backup"
            )
        if (
            replacement_backup.dump_sha256 != target.replacement["dump_sha256"]
            or replacement_backup.source_release_id
            != target.replacement["source_release_id"]
        ):
            raise StateAmbiguityError(
                "restore target replacement backup identity disagrees"
            )
        if target.replacement["source_release_id"] not in releases:
            raise StateAmbiguityError(
                "restore target replacement references an unknown source release"
            )


def _selected_link(
    paths: ManagedPaths, releases: Mapping[str, ReleaseRecord]
) -> str | None:
    ...
    try:
        ...
        raise StateAmbiguityError(
            "current selection points outside release root"
        ) from error
...
def _database_state(
    ...
    if set(database) - {"state", "applied_migrations", "initial_empty"}:
        raise StateAmbiguityError(
            "database observation contains duplicate or unknown facts"
        )
    ...
    try:
        ...
        raise StateAmbiguityError(
            "applied migrations are not sorted and unique"
        ) from None
    except ValueError:
        raise StateAmbiguityError("applied migrations are invalid") from None
    initial_empty = database.get("initial_empty", False)
    if type(initial_empty) is not bool or (
        initial_empty and (state != "ready" or migrations)
    ):
