...
def _redact_value(value: Any, redactor: Redactor, seen: dict[int, Any]) -> Any:
    ...
    if identity in seen:
        ...
        prior = seen[identity]
        return REDACTED if prior is _IN_PROGRESS else prior
...
class WorkflowResult:
    """The stable workflow report shared by all workflow entry points."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.exit_status, ExitStatus):
            raise TypeError("workflow result exit status must be typed")
        if self.process_status is not None and (
            type(self.process_status) is not int or not 0 <= self.process_status <= 255
        ):
            raise TypeError("workflow result process status must be an integer from 0 to 255")
...
def _result_fields(result: WorkflowResult | Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(result, Mapping):
        ...
        fields_map.setdefault("exit_status", ExitStatus.OK)
        return fields_map
...
def result_payload(result: WorkflowResult | Mapping[str, Any]) -> dict[str, Any]:
    ...
    exit_status = _result_exit_status(fields_map)
    return {
        "schema_version": 1,
        "command": _json_safe(fields_map["command"]),
        "environment": _json_safe(fields_map["environment"]),
        "status": "ok" if exit_status is ExitStatus.OK else "failed",
...
def result_exit_status(result: WorkflowResult | Mapping[str, Any]) -> ExitStatus:
    """Return the typed exit code carried by one already-renderable result."""

    return _result_exit_status(_result_fields(result))


def result_process_status(result: WorkflowResult | Mapping[str, Any]) -> int | None:
    """Return an exact interactive child status when the result carries one."""
...
def _result_exit_status(fields_map: Mapping[str, Any]) -> ExitStatus:
    status = fields_map.get("exit_status", ExitStatus.OK)
