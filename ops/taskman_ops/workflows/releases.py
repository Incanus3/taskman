...
def _request(operation: str, config: EnvironmentConfig) -> HostRequest:
    return helper_request(operation, config)
...
def _discovery(state: Mapping[str, object], warnings: Sequence[str]) -> DiscoveryResult:
    if not isinstance(state, Mapping) or set(state) - {"host_kind", "releases"}:
        raise _invalid_result()
    if type(state.get("host_kind")) is not str or state["host_kind"] not in {"empty", "manual", "managed", "staged"}:
        raise _invalid_result()
    ...
    if len({row["release_id"] for row in parsed}) != len(parsed):
        raise _invalid_result()
    _validate_release_rows(state["host_kind"], parsed)
