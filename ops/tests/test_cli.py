...
def test_dispatch_routes_rollback_to_the_compatibility_gated_workflow(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The CLI controller must route rollback through the compatibility gate."""

    environment = object()
    remote = object()
    seen: list[tuple[object, object, str, bool]] = []
    expected = WorkflowResult("rollback", "production", False, "confirmation-cancelled", {})
    ...
    monkeypatch.setattr(
        "taskman_ops.workflows.rollback.rollback",
    ...
    result = dispatch(Invocation("rollback", environment="production", release_id="0.2.0-aaaaaaaaaaaa-ubuntu26.04-amd64-otp27.3.4.6"))

    assert result is expected
    assert seen == [(remote, environment, "0.2.0-aaaaaaaaaaaa-ubuntu26.04-amd64-otp27.3.4.6", False)]
