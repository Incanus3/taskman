...
def test_rollback_refuses_preflight_before_discovery(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = rollback(object(), environment_config(), TARGET)
...
def test_rollback_confirms_the_discovered_target_and_exposes_only_final_public_facts(
    ...
    outcome = rollback(
        object(), environment_config(), TARGET, confirm=lambda plan: plan["target_release_id"] == TARGET
    )
    ...
    assert outcome.facts == {
        ...
        "service_state": "running",
        "database_state": "unchanged",
        "verification": successful_verification_report(TARGET),
    }
...
def test_rollback_preserves_final_manual_warning_and_release_exit_category(
    ...
    outcome = rollback(object(), environment_config(), TARGET, confirm=lambda _plan: True)
