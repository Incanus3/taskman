...
from taskman_ops.remote import run_interactive
...
def test_create_admin_uses_only_the_fixed_constrained_release_command_and_propagates_status() -> None:
    ...
    assert calls == [environment_config()]
...
def test_create_admin_session_allocates_a_strict_ssh_tty_without_credential_arguments_or_environment(
    ...
    assert shlex.split(ssh_argv[-1]) == list(CREATE_ADMIN_COMMAND)
...
@pytest.mark.parametrize("install_root", [
    "/srv/taskman",
    "/srv/taskman$(printf${IFS}CANARY)",
    "/srv/taskman;:",
])
def test_create_admin_preserves_the_validated_install_root_through_the_remote_shell(
    ...
    def runner(argv: list[str], **_kwargs: object) -> subprocess.CompletedProcess[bytes]:
        ...
        remote_command = " ".join(argv[argv.index(config.ssh_host) + 1:])
        shell = subprocess.run(
            ["/bin/sh", "-c", 'sudo() { printf "%s\\0" "$@"; }; ' + remote_command],
            capture_output=True, check=False,
        )
        observed.extend(shell.stdout.decode().rstrip("\0").split("\0"))
        return subprocess.CompletedProcess(argv, shell.returncode)
    ...
    assert observed == [
        ...
        f"--property=WorkingDirectory={install_root}/current",
        "--property=EnvironmentFile=/etc/taskman/taskman.env", "--",
        f"{install_root}/current/bin/create-admin",
    ]


def test_run_interactive_quotes_both_quote_types_with_a_validated_configuration(
    tmp_path: Path,
) -> None:
    """The SSH shell bridge must retain literal quote characters in argv values."""

    config = environment_config()
    literal = 'single\' and double" quote; $(printf CANARY)'
    ...
    def runner(argv: list[str], **_kwargs: object) -> subprocess.CompletedProcess[bytes]:
        if argv[0] == "ssh-keyscan":
            return subprocess.CompletedProcess(argv, 0, (key_line + "\n").encode(), b"")
        if argv[0] == "ssh-keygen":
            return subprocess.CompletedProcess(
                argv,
                0,
    ...
    assert observed == [literal.encode()]
...
try:
    ...
    raise SystemExit(90)
import shlex
remote_command = " ".join(sys.argv[separator + 2:])
if (
    sys.argv[1] != "-tt"
    or sys.argv[separator + 1] != os.environ["EXPECTED_SSH_HOST"]
    or shlex.split(remote_command) != expected
    or terminal_fds != [True, True, True]
):
    raise SystemExit(90)
os.execvpe("/bin/sh", ["/bin/sh", "-c", remote_command], os.environ)
""",
    )
    _write_executable(
        commands / "sudo",
...
"""
    ...
    assert shlex.split(invocation["argv"][-1]) == list(
        CREATE_ADMIN_COMMAND
    )
    assert result["stage"] == "administrator-command-failed"
    assert result["facts"] == {"remote_status": 37}
