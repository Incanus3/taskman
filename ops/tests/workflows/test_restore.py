...
def test_restore_dry_run_confirms_the_exact_backup_without_an_inspection_protocol(
    ...
    outcome = restore(object(), environment_config(), BACKUP, dry_run=True)
...
def test_restore_refuses_preflight_before_discovery(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "taskman_ops.workflows.restore.validate_operational_preflight",
    ...
    result = restore(object(), environment_config(), BACKUP)
...
def test_restore_sends_the_confirmed_source_and_returns_final_database_release_and_service_facts(
    ...
    outcome = restore(
        object(), environment_config(), BACKUP, confirm=lambda plan: plan["backup_id"] == BACKUP
    )
...
def test_restore_refuses_success_without_fresh_target_verification(
    ...
    outcome = restore(object(), environment_config(), BACKUP, confirm=lambda _plan: True)
