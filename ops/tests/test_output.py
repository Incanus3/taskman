...
from tests.support.secrets import no_registered_secrets_between_tests as no_secrets_between_tests
...
def test_repeated_empty_immutable_container_is_not_mistaken_for_a_cycle() -> None:
    """Python reuses the empty tuple, including as its own redacted result."""

    shared: tuple[()] = ()

    assert redact([shared, shared]) == [(), ()]
