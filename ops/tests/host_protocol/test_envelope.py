...
@pytest.mark.parametrize("operation", ("backup", "cleanup"))
def test_result_for_request_preserves_identity_and_explicit_evidence(operation: str) -> None:
    request = HostRequest.from_mapping(request_mapping(operation=operation))
    state = {"selected_release_id": None, "targets": []}
    ...
    state["targets"].append("later mutation")
    assert result.state["targets"] == ()


def test_result_for_request_defaults_to_no_warnings() -> None:
    request = HostRequest.from_mapping(request_mapping())
    assert HostResult.for_request(request, "refused", "unsafe", {}).warnings == ()
...
def test_result_for_request_keeps_protocol_validation(overrides: dict[str, object]) -> None:
    request = HostRequest.from_mapping(request_mapping())
    arguments = {"outcome": "succeeded", "message": "done", "state": {}, **overrides}
    with pytest.raises(ProtocolError):
        HostResult.for_request(request, **arguments)
