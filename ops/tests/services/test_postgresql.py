...
def test_postgresql_plan_uses_an_explicit_requested_ubuntu_package_track() -> None:
    ...
    plan = build_postgresql_plan(config(postgres_package_track="17"))
...
def test_native_configuration_script_has_fail_fast_config_and_hba_gates_before_restart() -> None:
    ...
    script = render_postgresql_native_configuration_script(build_postgresql_plan(config()))
...
def test_native_configuration_script_stops_before_restart_when_pg_conftool_fails(tmp_path: Path) -> None:
    ...
    _fake_command(bin_dir / "pg_lsclusters", "printf '16 main 5432 online postgres /data /log\\n'")
    _fake_command(bin_dir / "pg_conftool", "exit 1")
    _fake_command(bin_dir / "pg_ctlcluster", f"printf '%s\\n' \"$*\" >> {restart_log}")
...
def test_existing_incompatible_role_or_database_is_a_safety_refusal(
    ...
    with pytest.raises(OpsError) as raised:
        validate_existing_database_state(build_postgresql_plan(config()), role=role, database=database)
