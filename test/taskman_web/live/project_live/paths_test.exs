defmodule TaskmanWeb.ProjectLive.PathsTest do
  ...
  test "omits Task filters from browse, creation, and detail routes" do
    ...
    assert Paths.browse_path(project, task_list, true) == "/projects/11/lists/22"

    assert Paths.new_task_path(project, task_list, true, 33) ==
             "/projects/11/lists/22/tasks/new?parent_task_id=33"

    assert Paths.task_detail_path(project, nil, task, true) ==
             "/projects/11/tasks/33"
  end
  ...
  test "recognizes only the exact selected Task route" do
    ...
    assert Paths.selected_task_route?(
