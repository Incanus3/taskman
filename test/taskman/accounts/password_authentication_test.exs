  ...
  test "bootstrap passwords accept exactly 8 through 128 characters" do
    assert {:error, _error} =
             Accounts.bootstrap_admin(%{
               ...
               password_confirmation: String.duplicate("a", 7)
             })

    assert {:ok, _user} =
             Accounts.bootstrap_admin(%{
               email: "eight@example.com",
               password: String.duplicate("a", 8),
               password_confirmation: String.duplicate("a", 8)
             })

    assert {:ok, _user} =
             Accounts.bootstrap_admin(%{
               email: "one-twenty-eight@example.com",
               password: String.duplicate("a", 128),
               password_confirmation: String.duplicate("a", 128)
             })

    assert {:error, _error} =
             Accounts.bootstrap_admin(%{
               email: "one-twenty-nine@example.com",
               password: String.duplicate("a", 129),
               password_confirmation: String.duplicate("a", 129)
             })
  end

  test "bootstrap rejects a mismatched password confirmation" do
    assert {:error, _error} =
             Accounts.bootstrap_admin(%{
