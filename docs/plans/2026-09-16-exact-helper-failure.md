...
- [ ] Add an actual deploy runtime stop-error regression using the existing runtime seam. Raise
...
- [ ] Replace the native stop catch's category and directly related start category with canonical
...
- [ ] Replace deploy runtime `{ok: True}`/`{ok: False}` reports with complete valid reports using
...
- [ ] Run focused gate below; distinct scoped producer/fixture review; record evidence and commit
...
- [ ] Add direct shared-predicate tests for a valid failed subset, unchanged already-absent
...
- [ ] Move the existing predicate verbatim and export it; replace controller call/import and remove
  old private implementation. Keep full schema validation before request-membership validation.
- [ ] Run focused gate, independent scoped authority review, record evidence and commit
...
- [ ] Add focused tests using exact baseline states. Example baseline for deploy (reuse existing
...
- [ ] Validate request correlation before any salvage. Preserve existing cleanup successful-inspect
...
- [ ] Implement design precedence: cleanup report always ineligible; independently valid failed
...
- [ ] Reuse valid operation-owned fact group. Otherwise call existing safe observer once; validate
...
- [ ] Replace encoding fallback: first encode normal result; on failure, use validated proof and
...
- [ ] Add size proof tests using maximum valid escaped strings, 64 maximum-path targets, IDs and
...
- [ ] Exercise at least one actual isolated zipapp entrypoint with an invalid cooperative handler
...
- [ ] Run focused gate, distinct correctness review of this increment and its consumers; record
...
- [ ] Self-check every design acceptance row against tests and source, inspect net removed
...
- [ ] Run sequential complete local gates with no concurrent source edits/builds:
...
- [ ] Record exact verification, review dispositions and limitations in canonical owners. Close
...
- [ ] Inspect `but diff`, commit `Record verified exact helper failure recovery checkpoint`, confirm
