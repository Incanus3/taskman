...
- Production receives an immutable OTP release and explicit Ubuntu runtime prerequisites, not
  source, Mix, Node, controller Python packages, pyinfra, SOPS, age, or a container runtime.
  Ubuntu's `python3-minimal` is required for fixed repository-owned validation programs.
...
npm 9.2.0~ds3-1
Hex 2.5.1
Rebar3 3.24.0 (versioned binary with recorded SHA-512)
```
...
Invoke only `<validated-managed-root>/current/bin/create-admin` as `taskman`, with the working
directory derived from the same root and the root-only environment file. Do not accept an
arbitrary command.
...
docker buildx build --platform linux/amd64 \
  ...
  --file ops/builder/Containerfile .
```
