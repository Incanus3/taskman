...
- Install Alpine's `git` package before dependency resolution.
...
The workflow checks out the repository and restores its dependency cache before installing Git with
`apk add --no-cache git`. `config/test.exs` reads `POSTGRES_HOST` with `localhost` as its default so
the container job can use the service label without changing local development behavior. No
application source, production runtime configuration, or product behavior changes.

## Verification

- Parse the workflow as YAML.
- Confirm the workflow contains the x64 runner, pinned compatible image, checkout action, cache
  action, Git installation, and PostgreSQL service contract.
...
closing the follow-up. The runner, checkout action, and cache should remain unchanged during that
upgrade.
