...
`--dry-run` validates local inputs, decryptability, artifact metadata, SSH connectivity, supported
platform and DNS facts, runtime-file metadata and required keys without values, database access,
capacity, and the planned operations without remote mutation or confirmation. Restore additionally
performs fresh exact-dump validation. When no artifact was supplied, a dry run may create a local
artifact because the artifact is required for complete compatibility and migration planning.
...
- managed, release, deployment-state, and backup roots;
...
`release_root` and `deployment_root` are distinct descendants of `managed_root`. Those roots,
`backup_root`, and the derived `managed_root/current` path cannot be equal or have an
ancestor/descendant relationship; `backup_root` is also disjoint from `managed_root`. Every
configured or derived path is disjoint from Taskman's reserved configuration, state, lock, and
installed-program roots. Capabilities consume the exact validated roots: staging cleanup uses
`deployment_root/uploads`, while the constrained administrator working directory and executable
are derived from `managed_root/current`.
...
Node, Hex, Rebar3, and architecture inputs. Hex and Rebar3 installation uses exact versions rather
than a live latest resolver; the versioned Rebar3 binary is checked against its recorded SHA-512
digest. It executes the project-approved production sequence:
...
undeclared input. The production server does not receive source, Mix, Node, controller Python
packages, pyinfra, SOPS, age, or the build container. Ubuntu's distro `python3-minimal` is an
explicit host prerequisite used only to execute fixed repository-owned validation programs.
...
- pinned OTP, Elixir, Node, Hex, and Rebar3 identifiers;
...
The baseline installs Ubuntu's `python3-minimal`; existing-host operations require `python3` before
executing their fixed remote validation programs.
...
- selects the first release only through the canonical lock-held release transaction's distinct
  genesis integration.
...
The genesis integration accepts only an empty lifecycle. It creates a validated pre-activation
backup, applies initial migrations, selects and starts the candidate without stopping a nonexistent
current service, performs the normal verification contract, and publishes records with a `null`
previous release. Matching partial evidence is safely resumed, while a completed first activation
is a verified no-op. Once this transaction begins, release and database failures use the same
evidence and recovery contracts as a normal deployment.
...
- configured DNS, memory, disk, and database-capacity facts;
...
Instead, after verifying the supported platform, DNS, capacity, current symlink, release layout,
permissions, exact runtime-file metadata and key presence, runtime health, and local PostgreSQL
topology, it:
...
1. before dry-run output or typed confirmation, validates the exact recorded dump path, regular
   non-link type, recorded size, and body with `pg_restore --list`;
2. verifies sufficient space for the restored and retained databases;
3. after confirmation, locks all lifecycle operations and repeats the exact dump and capacity
...
- equality, ancestor, descendant, and reserved-root topology refusal;
...
- lock contention;
- no-current genesis activation, retry, and completed-rerun behavior;
- pre-mutation operational preflight and cross-command dry-run boundaries;
- fresh restore-dump validation before confirmation and again under lock;
